...
- \`docker run -p container_port:host_port image\`:  (lower case p) This will define the host port to map to the container port.`,
  ...
  'curl-missing-fail': {
    'title': 'Consider using a \`--fail\` switch when using curl',
    'description': 'Add --fail to curl so that the exit code is 0 > when something fails.',
    'category': 'Optimization'
  },
  'curl-not-secure': {
    'title': 'Consider using a secure connection when using curl',
    'description': 'Use https where possible to help mitigate man-in-the-middle attacks during build.',
    'category': 'Optimization'
  }
}
