  ...
  if (args) {
    ...
    switch (cmd) {
      ...
      case 'run':
        ...
        // parse curl command
        curl.curl_commands(commands).forEach(function (curl_command, index) {
          if (!curl.curl_hasfail(curl_command)) {
            items.push(messages.build(state.rules, 'curl-missing-fail', line));
          }
          if (!curl.curl_is_secure(curl_command)) {
            items.push(messages.build(state.rules, 'curl-not-secure', line));
          }
        });
        // parse apt commands
